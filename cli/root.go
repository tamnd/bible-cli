// Package cli assembles the bible command tree from the bible
...
// NewApp assembles the kit application from the bible domain. The
...
//
// To add a command, declare it in bible/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := bible.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(bible.Domain{}).Register(app)
