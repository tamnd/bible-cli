...
describes what users get. Pick a format with `-o`, or let bible choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bible <command> -o table   # aligned columns for reading
bible <command> -o jsonl   # one JSON object per line, for piping
bible <command> -o json    # a single JSON array
bible <command> -o csv     # spreadsheet friendly
bible <command> -o tsv     # tab-separated
bible <command> -o url     # just the URL column
bible <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bible <command> --fields id,title,url
```
...
```bash
bible <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bible <command>            # a table, because this is a terminal
bible <command> | wc -l    # JSONL, because this is a pipe
```
